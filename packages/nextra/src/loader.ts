...
const UNDERSCORE_APP_FILENAME: string =
  fs.readdirSync(PAGES_DIR).find((fileName) => /^_app\.(js|jsx|ts|tsx|md)$/.test(fileName)) || ''
...
const initGitRepo = (async () => {
  if (!IS_WEB_CONTAINER) {
    ...
    try {
      ...
      console.warn(`[nextra] Init git repository failed ${(error as Error).message}`)
    }
  }
  return {}
})()

async function loader(context: LoaderContext<LoaderOptions>, source: string): Promise<string> {
  const {
    isMetaImport = false,
    isPageImport = false,
    theme,
  ...
  if (mdxPath.includes('/pages/api/')) {
    console.warn(`[nextra] Ignoring ${mdxPath} because it is located in the "pages/api" folder.`)
    return ''
  }

  const { items, fileMap }: { items: PageMapItem[]; fileMap: FileMap } = IS_PRODUCTION
    ? pageMapCache.get()!
  ...
  const { result, title, frontMatter, structurizedData, searchIndexKey, hasJsxInH1, readingTime } =
  ...
  if (repository && gitRoot) {
    try {
      timestamp = await repository.getFileLatestModifiedDateAsync(path.relative(gitRoot, mdxPath))
    } catch {
...
${themeConfigImport ? '__nextra_internal__.themeConfig = __nextra_themeConfig' : ''}`
  ...
  const dynamicMetaModules = dynamicMetaItems
    .map(
      (descriptor) =>
        `[import(${JSON.stringify(descriptor.metaFilePath)}), ${JSON.stringify(descriptor)}]`,
    )
    .join(',')
