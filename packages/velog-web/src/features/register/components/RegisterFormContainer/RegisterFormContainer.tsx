...
import RegisterForm, { RegisterFormType } from '../RegisterForm/RegisterForm'
...
const RegisterFormContainer: React.FC<RegisterFormContainerProps> = () => {
  ...
  const query: { code: string | null; social: string | null } = {
  ...
  const [onGetRegisterToken, , registerToken] = useRequest<GetRegisterTokenResponse>(
  ...
  return (
    <RegisterForm
      loading={loading}
      onSubmit={onSubmit}
      fixedEmail={(registerToken && registerToken.email) || (socialProfile && socialProfile.email)}
      error={error}
      defaultInfo={
        socialProfile && {
